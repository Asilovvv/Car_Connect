...
public class ReviewServiceImpl implements ReviewService {
    ...
    @Override
    public List<ReviewResponse> makeReview(UUID userId, UUID carId, String comment, int rating) {
        User reviewer = userRepository.findById(userId).orElseThrow(() -> new CustomException("User not found", HttpStatus.NOT_FOUND));
        Car car = carRepository.findById(carId).orElseThrow(() -> new CustomException("Car not found", HttpStatus.NOT_FOUND));
